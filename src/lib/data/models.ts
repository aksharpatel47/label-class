...
export async function fetchPotentialUnlabeledPositivesData(
  ...
  labelName: string,
  modelId: number,
  leftThreshold: number = 1000,
  rightThreshold: number = 10000
) {
  ...
  const res = await db.execute(sql`
      ...
      select
        ...
        t.label_id,
        count(distinct case when ti.inference >= ${leftThreshold} and ti.inference <= ${rightThreshold} and tl.task_id is null then t.task_id end) as potential_positives,
